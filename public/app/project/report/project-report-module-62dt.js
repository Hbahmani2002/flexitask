...
define(["common/errorhandler", "knockout", "plugins/router", "durandal/activator", "plugins/http", "common/context", "moment", "common/utils"],
    function (errorhandler, ko, router, activator, http, context, moment, utils) {
        var ctor = function (requiredData, days) {
            var self = this;
            var tomorrow = moment().hour("0").minute("0").second("0").millisecond(0);
            var aWeekLater = moment().hour("0").minute("0").second("0").millisecond(0);
            var userDefinedDays = 7;
            if (parseInt(days, 10)) {
                userDefinedDays = parseInt(days);
            }
            else if (days === "0") {
                userDefinedDays = 1;
            }
            ...
            aWeekLater = moment(tomorrow).add(userDefinedDays, "days");

            var filteredData = _.filter(requiredData, function (t) {
                var dueDate = ko.unwrap(t.dueDate);
                if (dueDate != undefined && dueDate != null && ko.unwrap(t.taskType) === 0) {

                    dueDate = moment(dueDate).hour("0").minute("0").second("0").millisecond(0);
                    if (moment(dueDate).diff(tomorrow) > 0 && moment(dueDate).diff(aWeekLater) <= 0) {
                        return t;
                    }
