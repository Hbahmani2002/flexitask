...
define(["common/errorhandler", "knockout", "plugins/router", "durandal/activator", "plugins/http", "common/context", "moment", "common/utils"],
    function (errorhandler, ko, router, activator, http, context, moment, utils) {
        var ctor = function (requiredData) {
            var self = this;
            var today = new moment().hour("0").minute("0").second("0").millisecond(0);

            var filteredData = _.filter(requiredData, function (t) {
                var dueDate = ko.unwrap(t.dueDate);

                if (dueDate !== undefined && dueDate !== null && ko.unwrap(t.taskType) === 0 && (ko.unwrap(t.status) === 0 || ko.unwrap(t.status) === 1 || ko.unwrap(t.status) === 4 || ko.unwrap(t.status) === 8 || ko.unwrap(t.status) === 64)) {
