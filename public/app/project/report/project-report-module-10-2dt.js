...
define(["common/errorhandler", "knockout", "plugins/router", "durandal/activator", "plugins/http", "common/context", "moment", "common/utils"],
    function (errorhandler, ko, router, activator, http, context, moment, utils) {
        var ctor = function (requiredData, days) {
            var self = this;
            var tomorrow = moment().add(1, "days").hour("0").minute("0").second("0").millisecond(0);
            var daysLaterDefinedByUser = moment().hour("0").minute("0").second("0").millisecond(0);
            var userDefinedDays = 3;
            ...
            daysLaterDefinedByUser = moment(tomorrow).add(userDefinedDays, "days");
            ...
            var filteredData = _.filter(requiredData, function (t) {
                ...
                if (startDate != undefined && startDate != null && ko.unwrap(t.taskType) === 0) {
                    ...
                    startDate = moment(startDate).hour("0").minute("0").second("0").millisecond(0);

                    if (moment(startDate).diff(tomorrow) >= 0 && moment(startDate).diff(daysLaterDefinedByUser) <= 0) {
                        return t;
                    }
