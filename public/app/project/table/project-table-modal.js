define(["common/kendolib","jquery", "durandal/events", "common/autocomplete", "config", "common/helpers", "common/context", "i18n", "plugins/dialog", "plugins/http", "durandal/composition", "common/notifications", "durandal/app", "knockout", "common/errorhandler", "underscore", "common/utils"],
    function (kendo,$, events, autocomplete, config, helpers, context, i18n, dialog, http, composition, notifications, app, ko, errorhandler, _, utils) {
        ...
        taskTableModal.prototype.bindRecordForm = function () {
            var _this = this;
            var url = String.format("{0}/api/tasks/{1}/attachments?type=task/table&token={2}", config.serviceEndpoints.baseEndpoint, _this.projectId, context.authToken());
        ...
        taskTableModal.prototype.saveRecord = function () {
            ...
            var url = "";
            if (row.id) { // edit
                url = String.format("/api/tables/{1}/rows/{2}?projectId={0}", _this.projectId, _this.tableId, row.id);
                http.put(url, {
                    data: data
                ...
                url = String.format("/api/tables/{1}/rows?projectId={0}", _this.projectId, _this.tableId);
                http.post(url, {
                    data: data
                }).then(function (response) {
                    // add row to grid
        ...
        taskTableModal.prototype.getAttachmentDownloadUrl = function (attachment) {
            ...
            return String.format("/api/tasks/{0}/attachments/{1}/download?token={2}", _this.projectId, attachment.id, context.authToken());
        };
        ...
        taskTableModal.prototype.activate = function (settings) {
            var _this = this;
            var url = String.format("/api/tables/{1}?projectId={0}", _this.projectId, _this.tableId);
            return http.get(url)
            .then(function (response) {
        ...
        taskTableModal.prototype.bindKendoGrid = function () {
            var _this = this;
            var url = config.serviceEndpoints.baseEndpoint+ String.format("/api/tables/{1}/rows?projectId={0}", _this.projectId, _this.tableId);
            ...
            var columnProjectName = {
             ...
             columns.push(columnProjectId);

             columns.push(columnTaskId);

             columns.push(columnTaskName);

             columns.push(columnProjectName);
            ...
            columns.push({
                command: [ {
                    ...
                    click: function (e) {
                        ...
                        var url = String.format("/api/tables/{1}/rows/{2}?taskId={0}", data.taskId, _this.tableId, id);
            ...
            _this.gridRowTransformer = function (row) {
              ...
              x.projectId = row.project.id;
              x.taskId = row.task.id;
              x.projectName = row.project.name;
              x.taskName = row.task.name;
