    ...
    Plugin.prototype = {
        ...
        init: function () {
            ...
            var onStartEvent = function (e) {
                if (list.options.noDragDrop) {
                    return;
                }
                var handle = $(e.target);
                if (!handle.hasClass(list.options.handleClass) || handle.hasClass(list.options.noDragClass)) {
        ...
        dragStart: function (e) {
            ...
            this.initialParent = dragItem.parent().parent();
            ...
            this.initialPlace = $('<div class="initial-place"/>');
            this.placeEl.after(this.initialPlace);
        ...
        dragStop: function (e) {
            ...
            function doWork(revert) {
                if (revert) {
                    el[0].parentNode.removeChild(el[0]);
                    s.initialPlace.replaceWith(el);
                    s.placeEl.remove();
                } else {
                    el[0].parentNode.removeChild(el[0]);
                    s.placeEl.replaceWith(el);
                }



                if (s.hasNewRoot) {
                    if (s.options.fixed === true) {
                        s.restoreItemAtIndex(el);
                    }
                    else {
                        s.el.trigger("lostItem");
                    }
                    s.dragRootEl.trigger("gainedItem");
                }
                else {
                    s.dragRootEl.trigger("change");
                }

                s.dragEl.remove();
                s.options.callback.call(s, s.dragRootEl, el);
            ...
            if (this.options.validateDrag) {
                var target = this.hasNewRoot ? null : this.placeEl.parent().parent();
                var promise = $.Deferred();

                var result = this.options.validateDrag.call(this, this.dragRootEl, el, this.initialParent, target);
                if (typeof result == "object" && result && result.promise) {
                    promise = result;
                } else {
                    if (result) {
