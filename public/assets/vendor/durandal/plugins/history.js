...
define(['durandal/system', 'jquery'], function (system, $) {
    ...
    history.navigate = function(fragment, options) {
        ...
        if (options.trigger) {
            ...
            // BEGIN-OZY (handing active fragment change issuee)
            if (options.change) {
                var fragment = history.fragment = history.getFragment(fragment);

                return history.options.changeHandler ?
                    history.options.changeHandler(fragment) :
                    false;
            }
            // END-OZY
        }
